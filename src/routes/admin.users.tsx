...
import { fetchAllUsers, updateUserStatus } from "@/backend/users";
          ...
          <tbody className="divide-y divide-border">
            {users.map((u) => (
              <tr key={u.id} className="hover:bg-muted/40">
                ...
                <td className="px-4 py-3"><span className="chip">{u.base}</span></td>
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center justify-end gap-1">
                    <IconBtn onClick={() => toast.message(`Viewing ${u.name}`)} label="View"><Eye className="h-4 w-4" /></IconBtn>
                    <IconBtn onClick={() => toast.message(`Editing ${u.name}`)} label="Edit"><Pencil className="h-4 w-4" /></IconBtn>
                    <IconBtn
