...
import { submitEntry, type WorkingStatus, type FineCategory } from "@/backend/entries";
import { useMemo, useState } from "react";
import { Check, Save, Train, User, Phone, MapPin, IndianRupee, Briefcase, Cloud } from "lucide-react";
...
const WORKING_OPTIONS: WorkingStatus[] = ["VIRAT", "OPEN", "REST", "LEAVE", "VEDANT", "SQD", "LAP", "SICK"];
  ...
  return (
    <CollectorLayout>
      ...
      <Section title="Your Details" subtitle="Auto-filled from profile">
        <Grid>
          <Field icon={<User className="h-4 w-4" />} label="Collector">{profile.name}</Field>
          <Field icon={<MapPin className="h-4 w-4" />} label="Base">{profile.base}</Field>
          <Field icon={<Phone className="h-4 w-4" />} label="Mobile">{profile.mobile}</Field>
