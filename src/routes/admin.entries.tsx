      ...
      <div className="overflow-x-auto rounded-2xl border border-border bg-card shadow-card">
        <table className="w-full text-sm">
          <thead className="bg-muted/60 text-left text-xs uppercase tracking-wide text-muted-foreground">
            <tr>
              {["Date", "Collector", "Base", "Train", "Working", "A", "B", "C", "D", "E", "Smoke", "Cases", "Amount"].map((h) => (
          ...
          <tbody className="divide-y divide-border">
            {rows.map(({ e, u }) => (
              <tr key={e.id} className="hover:bg-muted/40">
                ...
                <td className="whitespace-nowrap px-3 py-3"><span className="chip">{u?.base}</span></td>
                <td className="whitespace-nowrap px-3 py-3 font-mono font-semibold">{e.trainNumber}</td>
