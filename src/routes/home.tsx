...
import { fetchMyEntries } from "@/backend/entries";
import { formatINR } from "@/lib/format";
import { Plus, FileText, ClipboardList, MessageSquare, IndianRupee, Briefcase, ChevronRight } from "lucide-react";
...
function HomePage() {
  ...
  const { data: entries = [] as import("@/backend/entries").Entry[] } = useQuery<import("@/backend/entries").Entry[]>({
  ...
  const actions = [
    { to: "/entry/new", icon: Plus, label: "New Daily Entry", desc: "Record train, cases & fines", tone: "primary" },
    { to: "/complaint/new", icon: FileText, label: "Submit Complaint", desc: "File an on-duty complaint", tone: "soft" },
    { to: "/entries", icon: ClipboardList, label: "My Entries", desc: "View past daily entries", tone: "soft" },
    { to: "/complaints", icon: MessageSquare, label: "Complaint History", desc: "Track your complaints", tone: "soft" },
  ] as const;
          ...
          return (
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className={`truncate text-xs ${isPrimary ? "opacity-80" : "text-muted-foreground"}`}>
                  {a.desc}
                </div>
              </div>
              <ChevronRight className={`h-5 w-5 ${isPrimary ? "opacity-80" : "text-muted-foreground"}`} />
            </Link>
          );
        })}
      </section>
