...
async function getServerEntry() {
	if (!serverEntryPromise) serverEntryPromise = import("./server-AsDQMEYY.mjs").then((m) => m.default ?? m);
...
var server_default = { async fetch(request, env, ctx) {
	try {
		...
		return new Response(`<html><body><h1>SSR Render Error</h1><pre>${stack}</pre></body></html>`, {
			status: 500,
			headers: { "content-type": "text/html; charset=utf-8" }
		});
	}
